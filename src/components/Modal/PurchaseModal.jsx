...
const PurchaseModal = ({ closeModal, isOpen, plant,refetch }) => {
  const {name,category,description,price,quantity,image,seller,_id} = plant
  const {user} = useAuth()
  ...
  const handleQuentity=(value)=>{
    ...
    if(value < 1){
      setTotalQuantity(quantity)
      return toast.error(`Quantity can't be lees than 1`)
    }
    setTotalQuantity(value)
    setTotalPrice(price * value)
    setPurchaseInfo(pre=>{
      return {...pre, quantity: value, price: price * value}
    })
  }
  // Total Price Calculation
   const handlePurchase = async () => {
    ...
    try{
      ...
      //decrease quantity in plants collection
            ...
            >
              <DialogPanel className='w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all'>
                ...
                <input
                  ...
                  value={totalQuantity}
                  onChange={(e) => handleQuentity(parseInt(e.target.value))}
                ...
                <input
                  ...
                  name="address"
                  id="address"
                  onChange={(e)=>setPurchaseInfo(pre=>{
